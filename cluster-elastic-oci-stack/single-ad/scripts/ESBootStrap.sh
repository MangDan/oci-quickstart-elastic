...
yum install -y https://artifacts.elastic.co/downloads/elasticsearch/elasticsearch-7.6.2-x86_64.rpm
...
echo "cluster.initial_master_nodes : [\"$esmasternode1\",\"$esmasternode2\",\"$esmasternode3\"]" >>/etc/elasticsearch/elasticsearch.yml
...
echo "xpack.monitoring.enabled: true" >>/etc/elasticsearch/elasticsearch.yml
...
yum install -y https://artifacts.elastic.co/downloads/kibana/kibana-7.6.2-x86_64.rpm
yum install -y https://artifacts.elastic.co/downloads/logstash/logstash-7.6.2.rpm
...
echo "xpack.monitoring.enabled: true" >>/etc/elasticsearch/elasticsearch.yml
echo "xpack.monitoring.collection.enabled: true" >>/etc/elasticsearch/elasticsearch.yml
mv /etc/logstash/logstash.yml /etc/logstash/logstash.yml.original
...
echo "xpack.monitoring.elasticsearch.hosts: [\"$esmasternode1:9200\",\"$esmasternode2:9200\",\"$esmasternode3:9200\"]" >>/etc/logstash/logstash.yml
...
output {
  elasticsearch {
    hosts => [\"$esdatanode1:9200\",\"$esdatanode2:9200\",\"$esdatanode3:9200\"]
...
firewall-offline-cmd --add-port=9600/tcp
firewall-offline-cmd --add-port=5044/tcp
systemctl restart firewalld
}
